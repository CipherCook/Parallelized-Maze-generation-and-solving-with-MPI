...
vector<vector<char>> dijkstra(vector<vector<char>> &maze, int rank, int comm_sz, int flag) {
    ...
    if(flag == PARALLEL){
        ...
        // Using the same pseudo code as taugh in class.
        vector<vector<int>> dist(MAZE_SIZE*2, vector<int>(2 * MAZE_SIZE, INT_MAX));
        // vector<vector<bool>> visited(MAZE_SIZE*2, vector<bool>(2 * MAZE_SIZE, false));
        priority_queue<
        ...
        while(sz > 0){
            ...
            pair<int, pair<int, int>> global_adjacent[my_sz];
            MPI_Barrier(MPI_COMM_WORLD);
